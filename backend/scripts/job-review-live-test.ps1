...
function Invoke-GrpcCall([string]$addr,[string]$method,[hashtable]$headers,[string]$json,[switch]$UseReviewProto){
  ...
  $args=@('-plaintext')
  if($UseReviewProto){
    $args += @('-import-path','backend/api/proto','-proto','review/v1/review.proto')
  }
...
$results | ConvertTo-Json -Depth 5 | Set-Content backend/scripts/job-review-live-test-summary.json
Write-Host 'summary_file=backend/scripts/job-review-live-test-summary.json'
