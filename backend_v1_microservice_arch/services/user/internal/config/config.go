...
type PortfolioStorageConfig struct {
	Provider     string
	Bucket       string
	Endpoint     string
	Region       string
...
func LoadFromEnv() (Config, error) {
	cfg := Config{
		...
		PortfolioStorage: PortfolioStorageConfig{
			...
			Endpoint:     strings.TrimSpace(getEnv("USER_PORTFOLIO_STORAGE_ENDPOINT", "localhost:9000")),
			Region:       strings.TrimSpace(getEnv("USER_PORTFOLIO_STORAGE_REGION", "us-east-1")),
			AccessKey:    strings.TrimSpace(os.Getenv("USER_PORTFOLIO_STORAGE_ACCESS_KEY")),
			SecretKey:    strings.TrimSpace(os.Getenv("USER_PORTFOLIO_STORAGE_SECRET_KEY")),
			UseSSL:       getEnvBool("USER_PORTFOLIO_STORAGE_USE_SSL", false),
			PathStyle:    getEnvBool("USER_PORTFOLIO_STORAGE_PATH_STYLE", true),
			CreateBucket: getEnvBool("USER_PORTFOLIO_STORAGE_CREATE_BUCKET", true),
		},
...
func (c PortfolioStorageConfig) Validate() error {
	if c.Provider != "minio" {
		return fmt.Errorf("USER_PORTFOLIO_STORAGE_PROVIDER must be 'minio'")
	}
	if c.Bucket == "" {
		return fmt.Errorf("USER_PORTFOLIO_STORAGE_BUCKET is required")
	}
	if c.Endpoint == "" {
		return fmt.Errorf("USER_PORTFOLIO_STORAGE_ENDPOINT is required")
	}
	if c.AccessKey == "" {
		return fmt.Errorf("USER_PORTFOLIO_STORAGE_ACCESS_KEY is required")
	}
	if c.SecretKey == "" {
		return fmt.Errorf("USER_PORTFOLIO_STORAGE_SECRET_KEY is required")
	}
	if c.Region == "" {
		return fmt.Errorf("USER_PORTFOLIO_STORAGE_REGION is required")
	}
	return nil
}
