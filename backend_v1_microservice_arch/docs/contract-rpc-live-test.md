...
```powershell
powershell -ExecutionPolicy Bypass -File ".\backend_v1_microservice_arch\scripts\contract-rpc-live-test.ps1"
```

Optional flags:

```powershell
powershell -ExecutionPolicy Bypass -File ".\backend_v1_microservice_arch\scripts\contract-rpc-live-test.ps1" -SkipComposeBuild
powershell -ExecutionPolicy Bypass -File ".\backend_v1_microservice_arch\scripts\contract-rpc-live-test.ps1" -SkipMigrations
```

What it does:
1. Verifies tools (`docker`, `grpcurl`, `curl.exe`).
2. Starts backend stack with Docker Compose.
3. Runs migrations (`backend_v1_microservice_arch/scripts/migrate-all.ps1`) unless skipped.
...
7. Executes all `ContractService` RPCs using `backend_v1_microservice_arch/scripts/contract-rpc-cases.json`.
...
9. Writes JSON summary to:
   - `backend_v1_microservice_arch/scripts/contract-rpc-live-test-summary.json`

## Required environment
Set in `backend_v1_microservice_arch/.env`:
...
`backend_v1_microservice_arch/scripts/contract-rpc-cases.json` defines each RPC scenario with:
- `rpc`
...
1. Start stack:
```powershell
cd "backend_v1_microservice_arch"
docker compose up --build -d
.\scripts\migrate-all.ps1
```

2. Bootstrap auth tokens:
- Use Postman collection `backend_v1_microservice_arch/docs/postman/proposal-gateway.postman_collection.json`
- Use env `backend_v1_microservice_arch/docs/postman/proposal-local.postman_environment.json`
- Run:
...
- **`JOBCONNECT_INTERNAL_CALLER_SECRET` missing**
  - Add it to `backend_v1_microservice_arch/.env`, then rerun.
