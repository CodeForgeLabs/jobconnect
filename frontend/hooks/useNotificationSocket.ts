...
type NotificationHandler = (
  msg: IncomingNotificationWSMessage | unknown,
) => void;

const playNotificationSound = () => {
  try {
    // Path points directly to public/sound/notification.mp3
    // Change "notification.mp3" to match your exact file name!
    const audio = new Audio("/sound/notification.mp3");
    audio.volume = 0.4; // 40% volume so it isn't too loud
    audio.play().catch((err) => {
      // This catches the browser's autoplay block rule safely
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (
            ...
            // 🔊 Play sound when a system notification arrives
